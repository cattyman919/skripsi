...
\author{
    ...
    \IEEEauthorblockN{
        \authorOne\IEEEauthorrefmark{1}, % Menambahkan \IEEEauthorrefmark{2} untuk simbol † dan
        \authorTwo\IEEEauthorrefmark{1}  % Menambahkan \IEEEauthorrefmark{2} untuk simbol †
    }

    \IEEEauthorblockA{
        \IEEEauthorrefmark{1}\textit{\journalAffiliation} \\ % Afiliasi didahului oleh simbol †
        % Baris email bisa tetap di sini atau dipindahkan ke \thanks jika diinginkan
        % Untuk lebih mirip gambar, email mungkin lebih baik di \thanks atau dihilangkan dari blok utama ini jika mengikuti contoh gambar secara ketat.
        % Namun, untuk standar IEEE, email seringkali disertakan. Kita pertahankan untuk saat ini.
        Emails: \{\authorOneEmail, \authorTwoEmail\}
    }
}
...
\input{src/results_discussion.tex}
