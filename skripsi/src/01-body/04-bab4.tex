...
if(USE_VL_MACRO)
  ...
  \item \bo{Proses Kompilasi dan Virtualisasi Aktual:}
      \begin{itemize}
	      \item Build Asli: Jalankan \code{cmake -G Ninja -B build\_asli ..} diikuti \code{ninja -C build\_asli}. Ini menghasilkan \textit{executable} asli di \code{build\_asli/bin/}.
	      \item Build Intermediate (\texttt{*\_vm.exe}): Jalankan \code{cmake -G Ninja -B build\_vm -DUSE\_VL\_MACRO=1 ..} diikuti \code{ninja -C build\_vm}. Ini menghasilkan \textit{executable intermediate} yang berisi penanda VxLang di \code{build\_vm/bin/}, contohnya \code{console\_vm.exe}.
	      \item Proses Virtualisasi VxLang (\texttt{*\_vxm.exe}): Jalankan \textit{tool command-line} \texttt{vxlang.exe} secara langsung pada setiap file \textit{intermediate} yang relevan di direktori \texttt{build\_vm/bin/}. Contoh:
            \begin{verbatim}
cd build_vm/bin
../../vxlang.exe console_vm.exe
../../vxlang.exe app_qt_vm.exe
../../vxlang.exe Lilith_Client_vm.exe
REM ... dan seterusnya untuk target lain ...
            \end{verbatim}
            Proses ini secara otomatis menghasilkan file \textit{executable} akhir yang telah tervirtualisasi dengan nama \texttt{(nama\_target)\_vxm.exe} di direktori yang sama (misalnya, \texttt{console\_vxm.exe}, \texttt{app\_qt\_vxm.exe}, \texttt{Lilith\_Client\_vxm.exe}).
      \end{itemize}
\end{enumerate}
...
\subsection{Persiapan \textit{Executable} Lilith}
...
\begin{enumerate}
    ...
    \item \textbf{Penerapan VxLang:}
        \begin{itemize}
            ...
            \item \textbf{Proses Virtualisasi VxLang:} \textit{Executable intermediate} (\f{Lilith\_Client\_vm.exe}) kemudian diproses langsung menggunakan \textit{tool command-line} \texttt{vxlang.exe} (misalnya, \texttt{vxlang.exe Lilith\_Client\_vm.exe}), menghasilkan \textit{executable} Lilith klien yang telah tervirtualisasi dengan nama \f{Lilith\_Client\_vxm.exe}.
        \end{itemize}
