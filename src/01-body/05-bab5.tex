...
Dalam konteks kode di atas, pemanggilan \code{memcmp} (di \texttt{14000320b}) membandingkan password yang dimasukkan pengguna dengan nilai \textit{hardcoded}. Hasilnya diperiksa oleh \code{TEST EAX,EAX} (\texttt{140003216}). Jika password tidak sama, \code{EAX} tidak akan nol, dan instruksi \code{JNZ} (\texttt{140003218}, Jump if Not Zero) akan mengalihkan eksekusi ke \code{LAB\_140003226}, di mana pesan "Authentication Failed" ditampilkan. Untuk memvalidasi potensi celah keamanan, instruksi \code{JNZ} ini dapat diubah (\f{patched}) menjadi \code{JZ} (Jump if Zero) atau \code{NOP} (No Operation) untuk melewati pemeriksaan. Konteks assembly yang lebih lengkap dapat dilihat pada).
---
